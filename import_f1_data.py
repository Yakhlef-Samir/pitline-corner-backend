...
from app.repositories.f1 import circuit as circuit_repo, driver as driver_repo, race as race_repo
    ...
    try:
        ...
        for _, event in schedule.iterrows():
            if event['EventName'] and event['Location']:
                ...
                existing = await circuit_repo.get_by_name(db, name=event['CircuitShortName'])
                if existing:
                    continue
        ...
        mock_circuits = [
            ...
            {"name": "Jeddah", "country": "Saudi Arabia", "length_km": 6.174, "turns": 27},
            {"name": "Albert Park", "country": "Australia", "length_km": 5.278, "turns": 14},
...
async def import_drivers(db: AsyncSession):
    ...
    try:
        ...
        for driver_info in session.drivers:
            ...
            existing = await driver_repo.get_by_number(db, driver_number=driver_info['DriverNumber'])
            if existing:
                continue
            ...
            drivers_added += 1
            print(f"  ✅ Added driver: {driver_info['FirstName']} {driver_info['LastName']} ({driver_info['Code']})")
        ...
        mock_drivers = [
            {"driver_number": 1, "code": "VER", "first_name": "Max", "last_name": "Verstappen", "team": "Red Bull", "country": "NED"},
            {"driver_number": 11, "code": "PER", "first_name": "Sergio", "last_name": "Perez", "team": "Red Bull", "country": "MEX"},
            {"driver_number": 16, "code": "LEC", "first_name": "Charles", "last_name": "Leclerc", "team": "Ferrari", "country": "MON"},
            {"driver_number": 55, "code": "SAI", "first_name": "Carlos", "last_name": "Sainz", "team": "Ferrari", "country": "ESP"},
            {"driver_number": 4, "code": "NOR", "first_name": "Lando", "last_name": "Norris", "team": "McLaren", "country": "GBR"},
            {"driver_number": 81, "code": "PIA", "first_name": "Oscar", "last_name": "Piastri", "team": "McLaren", "country": "AUS"},
            {"driver_number": 44, "code": "HAM", "first_name": "Lewis", "last_name": "Hamilton", "team": "Mercedes", "country": "GBR"},
            {"driver_number": 63, "code": "RUS", "first_name": "George", "last_name": "Russell", "team": "Mercedes", "country": "GBR"},
        ]
        
        drivers_added = 0
        for driver_data in mock_drivers:
            existing = await driver_repo.get_by_number(db, driver_number=driver_data['driver_number'])
            if not existing:
                await driver_repo.create(db, obj_in=driver_data)
                drivers_added += 1
                print(f"  ✅ Added mock driver: {driver_data['first_name']} {driver_data['last_name']} ({driver_data['code']})")
    ...
    try:
        ...
        for _, event in schedule.iterrows():
            if event['RoundNumber'] and event['Session5Date']:  # Race round and date
                ...
                # Find circuit
                circuit_obj = await circuit_repo.get_by_name(db, name=event['CircuitShortName'])
                if not circuit_obj:
                    print(f"  ⚠️  Circuit not found: {event['CircuitShortName']}, skipping race")
                    continue
                ...
                races_added += 1
                print(f"  ✅ Added race: {event['EventName']} (Round {event['RoundNumber']})")
        ...
        mock_races = [
            {"season": 2024, "round": 1, "name": "Bahrain Grand Prix", "circuit_name": "Bahrain", "country": "Bahrain", "date": datetime.fromisoformat("2024-03-02T15:00:00"), "status": "completed"},
            {"season": 2024, "round": 2, "name": "Saudi Arabian Grand Prix", "circuit_name": "Jeddah", "country": "Saudi Arabia", "date": datetime.fromisoformat("2024-03-09T15:00:00"), "status": "completed"},
            {"season": 2024, "round": 3, "name": "Australian Grand Prix", "circuit_name": "Albert Park", "country": "Australia", "date": datetime.fromisoformat("2024-03-24T06:00:00"), "status": "completed"},
        ]
        
        races_added = 0
        for race_data in mock_races:
            # Find circuit
            circuit_obj = await circuit_repo.get_by_name(db, name=race_data['circuit_name'])
            if not circuit_obj:
                print(f"  ⚠️  Circuit not found: {race_data['circuit_name']}, skipping race")
                continue
            
            # Remove circuit_name from race_data
            race_data_clean = {k: v for k, v in race_data.items() if k != 'circuit_name'}
            ...
            races_added += 1
            print(f"  ✅ Added mock race: {race_data['name']} (Round {race_data['round']})")
