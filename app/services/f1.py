...
class DriverService:
    ...
    @staticmethod
    async def get_driver_by_id(db: AsyncSession, driver_id: int) -> Optional[DriverResponse]:
        """Get a specific driver by ID"""
        return await driver.get(db, id=driver_id)

    @staticmethod
    async def search_drivers(db: AsyncSession, query: str, limit: int = 10) -> DriverList:
...
class PitStopService:
    ...
    async def get_race_pit_stops(
        ...
        pit_stops = await pit_stop.get_race_pit_stops(db, race_id=race_id, driver_id=driver_id)
