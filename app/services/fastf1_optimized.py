...
class FastF1Service:
    ...
    async def load_race_session(self, year: int, gp_name: str, session_type: str = 'R'):
        ...
        session = await loop.run_in_executor(
            None,
            lambda: fastf1.get_session(year, gp_name, session_type)
        )
    ...
    async def import_race(self, db: AsyncSession, year: int, round_num: int, gp_name: str):
        ...
        try:
            ...
            for idx, lap in laps.iterrows():
                lap_data = LapData(
                    ...
                    lap_time_seconds=lap['LapTime'].total_seconds() if pd.notna(lap['LapTime']) else 0,
                    ...
                    tire_compound=lap['Compound'] if pd.notna(lap['Compound']) else 'UNKNOWN',
    ...
    async def _get_driver_id(self, db: AsyncSession, driver_code: str) -> int:
        ...
        result = await db.execute(
            select(Driver.id).where(Driver.code == driver_code)
        )
